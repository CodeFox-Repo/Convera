 ...
 * the renderer's Dexie. Exits non-zero when the turn ends without speaking —
 * the same condition the agent host reports as "completed a direct offer
...
if (!sent.length) console.log("RAW TEXT:", text.trim().slice(0, 400) || "(empty)");
