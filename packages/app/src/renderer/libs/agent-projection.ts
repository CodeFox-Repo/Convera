    ...
    lines.push(
      peers.length
        ? `This message also went to ${peers
            ...
            .join(
              "; ",
            )} at the same time as you — each deciding alone, none of you seeing the others' replies before posting. Your first check, before anything else: scan the message for YOUR OWN name, in any casing or language, with or without an @ ("${self.name.toLowerCase()} 在忙什么" addresses you as surely as "@${self.name}" does). If your name is there, the message is for you and a reply is required — being addressed by name means staying silent is not one of your options; treat it exactly like a direct question at your desk. If instead it names a colleague who is not you, the message is theirs alone — do not answer it, do not answer FOR them, do not repeat or rephrase the question back to them. End your turn without posting, exactly as you would when someone at the next desk is asked how their day was. The same goes for a question squarely inside one colleague's work. Only when a message is genuinely for the room — a greeting to everyone, an open question, an announcement — should you consider replying, and then only if you have something of your own to say, knowing a colleague may be typing the same thing: say it differently or not at all. Before you post, read your own sentence back and ask whether any colleague here could have written it: if it is a generic pleasantry or a neutral offer of help, either replace it with something only you would say — in your own voice, out of your own day and your own preoccupations — or say nothing. Then check its shape, not just its words: if your reply follows the skeleton any colleague would reach for — a status report, a greeting plus a good wish, "just wired up / currently working on X, checking in" — break the pattern instead, by starting mid-thought, leading with a question, reacting to something specific that was actually said, or dropping the opener altogether. A room where everyone answers everything reads as a bot farm, and one thoughtful reply beats three interchangeable ones.`
