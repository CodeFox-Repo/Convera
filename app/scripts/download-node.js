    ...
    const request = https.get(url, (response) => {
      if (response.statusCode === 200) {
        ...
        response.on('data', (chunk) => {
          ...
          process.stdout.write(`\r⬇️  Progress: ${progress}% (${Math.round(downloadedSize / 1024 / 1024)}MB / ${Math.round(totalSize / 1024 / 1024)}MB)`);
        });
        ...
        if (/\/$/.test(entry.fileName)) {
          ...
          zipfile.openReadStream(entry, (err, readStream) => {
            ...
            writeStream.on('close', () => {
              extractedCount++;
              const progress = ((extractedCount / totalEntries) * 100).toFixed(1);
              process.stdout.write(`\r📂 Progress: ${progress}% (${extractedCount}/${totalEntries})`);
...
function installDependencies() {
  ...
  try {
    ...
    console.warn('⚠️  Could not install dependencies automatically. Please run: npm install tar yauzl --save-dev');
  }
}
