...
async function main() {
  ...
  try {
    ...
    try {
      ...
      console.warn('⚠️  Could not install dependencies automatically. Please run manually: npm install tar yauzl --save-dev');
    }
    
    // Install MCP packages for portable Node.js
    console.log('🔧 Installing MCP packages for portable Node.js...');
    const portableNodeBin = path.join(__dirname, '..', 'resources', 'node', 'bin', 'npm');
    ...
    if (fs.existsSync(portableNodeBin)) {
      try {
        const env = {
          ...
          NPM_CONFIG_GLOBAL_FOLDER: path.join(portableNodeDir, 'lib', 'node_modules'),
          NPM_CONFIG_CACHE: path.join(portableNodeDir, '.npm'),
        };
        
        console.log('📦 Installing @foxychat-mcp/apple-calendar...');
        execSync(`"${portableNodeBin}" install -g @foxychat-mcp/apple-calendar`, { 
          stdio: 'inherit',
          env
        });
        
        console.log('📦 Installing @foxychat-mcp/apple-imessages...');
        execSync(`"${portableNodeBin}" install -g @foxychat-mcp/apple-imessages`, { 
          stdio: 'inherit',
          env
        });
        ...
        console.warn('⚠️  Could not install MCP packages in portable Node.js:', error.message);
      }
    } else {
      console.warn('⚠️  Portable Node.js not found, skipping MCP package installation.');
    }
    ...
    if (fs.existsSync(installScript)) {
      try {
        ...
        console.warn('⚠️  Could not run system MCP package installation script.');
      }
    }
