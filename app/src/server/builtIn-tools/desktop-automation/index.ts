 ...
 *
 * A comprehensive set of tools for desktop automation using nut.js
 * Allows AI agents to interact with the user's computer via mouse, keyboard, and screen operations
 */
...
export const mouseGetPosition = tool({
...
export const mouseButtonControl = tool({
  ...
  execute: async ({
    ...
    try {
      const filePath = path.join(
        os.tmpdir(),
        `desktop_automation_screenshot_${Date.now()}.png`,
      );
      ...
      // Read the image and get screen info
...
export const screenInfo = tool({
  description: "Get screen dimensions and information.",
  parameters: z.object({}),
  execute: async () => {
    return await getScreenInfo(nutjsAvailable);
  },
});

// Tool 12: Screen Highlight
export const screenHighlight = tool({
...
export const getWindowsList = tool({
...
export const getActiveWindowInfo = tool({
...
export const windowControl = tool({
...
export const waitForImage = tool({
...
export const mouseMovePath = tool({
...
export const systemCommand = tool({
  description:
...
console.log(
  `📋 Desktop Automation Tools initialized with nutjs support: ${nutjsAvailable}`,
);
console.log(`🔐 macOS permissions support: ${permissionsAvailable}`);
