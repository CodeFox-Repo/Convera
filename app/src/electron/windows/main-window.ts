...
function setupWindowEventHandlers(window: BrowserWindow, route?: string) {
  window.webContents.on("did-finish-load", () => {
    console.log(
      `Main page loaded in main window, redirecting to ${route || "/"}...`,
    );
    ...
    window.webContents
      ...
      if (window.router) {
        ...
        window.router.navigate({ to: "${route || "/"}" });
      } else {
