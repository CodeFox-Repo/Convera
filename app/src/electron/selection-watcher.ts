    ...
    try {
      ...
      if (getWindows) {
        try {
          ...
          if (activeWindow) {
            appName =
              activeWindow.owner?.name || activeWindow.title || "Unknown";
            appId =
              activeWindow.owner?.bundleId ||
              activeWindow.owner?.processId?.toString();
          }
        } catch (error) {
...
async function pollClipboardChanges(): Promise<void> {
  try {
    ...
    if (chatWindow && !chatWindow.isDestroyed()) {
      console.log(
        `✅ Clipboard-detected selection from ${appName}: "${currentText.slice(0, 50)}..."`,
      );
...
function checkMacOSAccessibilityPermission(): boolean {
  ...
  try {
    ...
    if (!hasPermission) {
      console.log(
        "⚠️ macOS Accessibility permission required for real-time selection detection",
      );
      ...
      if (!permissionGranted) {
        ...
        console.log(
          "💡 Please enable Accessibility access for FoxyChat in System Preferences > Security & Privacy > Privacy > Accessibility",
        );
        return false;
      }
    }
  ...
  try {
    ...
    if (uiohook && hasAccessibilityPermission) {
      try {
        ...
        uiohook.on("input", async (event: any) => {
          ...
          if (eventType === uiohookNapi.EventType.EVENT_MOUSE_RELEASED) {
            ...
            if (isClickOnFoxyChat) {
              console.log(
                "🏠 Click within FoxyChat - keeping current selection state",
              );
              return;
            }
            // 延迟一点检查，让选区完成
        ...
        uiohook.start();
        console.log(
          "✅ uiohook-napi started for real-time selection detection",
        );
        console.log(
          "🎯 Using mouse event detection - clipboard polling disabled",
        );
      } catch (error) {
      ...
      console.log(
        "⚠️ No Accessibility permission - using clipboard polling only",
      );
      console.log(
        "💡 To enable real-time selection detection, grant Accessibility permission and restart FoxyChat",
      );
      // 启动轮询
      pollInterval = setInterval(pollClipboardChanges, POLL_INTERVAL_MS);
    } else {
