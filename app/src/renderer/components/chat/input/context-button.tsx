...
const FileBadge = ({ file, onRemove }: { file: File; onRemove: () => void }) => {
...
export function ContextButtons({
  ...
  const remainingFiles = attachments.length > MAX_VISIBLE_FILES ? attachments.length - MAX_VISIBLE_FILES : 0;
    ...
    <div className="h-7.5 pt-1 overflow-hidden">
      <div className="px-2 w-full overflow-x-auto overflow-y-hidden whitespace-nowrap hide-scrollbar">
        <div className="inline-flex items-center gap-1 h-7">
          ...
          {remainingFiles > 0 && (
            <RemainingFilesBadge count={remainingFiles} />
          )}
        </div>
      </div>
    </div>
